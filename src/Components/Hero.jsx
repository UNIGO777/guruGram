...
const Hero = () => {
  ...
  useEffect(() => {
    if (isMobile || animationComplete) return; // Don't add event listeners for mobile devices or after animation
    ...
    const handleWheel = (e) => {
      ...
      if (scrollProgress >= 1 && !animationComplete) {
        ...
        // Remove event listeners to allow normal scrolling
      ...
      // Only prevent scrolling during animation
  ...
  return (
     <section className='min-h-screen max-h-screen flex items-center justify-center relative w-full overflow-hidden'>
        <div className='w-full absolute -bottom-3 z-20'>
            <img src={HeroImg} alt="Hero Section" className='w-full h-full object-cover' />
        </div>
        <div ref={leavesRef} className='w-full h-full hidden md:block absolute -bottom-0 z-50 transition-all duration-100'>
            <img src={LeavesOverlay} alt="Leaves Overlay" className='w-full h-full object-cover' />
        </div>
        ...
        <div  className='w-full absolute top-[15vh] z-10 left-[0vw] md:hidden block'>
            <img src={Cloudes} alt="Hero Section" className='w-full h-full object-cover' />
        </div>
